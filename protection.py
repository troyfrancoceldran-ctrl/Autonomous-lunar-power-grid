...
# @note calls that an upper bound, and the bound is loose — on the battery
# feeder it overstates by nearly 5x. Three limits replace the hand-wave, and
# only one of them is an impedance:
#
#   battery     IMPEDANCE-limited   OCV(soc) / (R_internal + R_cable)
#   pv          PHYSICS-limited     Isc, which a cell cannot exceed
#   converter   CONTROL-limited     current limit x rated
#
# The battery's 14.3 mohm is 0.7x the cable's own, so impedance is a real
# correction rather than a rounding term. But CONTROL is the larger one: a
# converter regulates current and refuses to exceed its limit, which has
# nothing to do with resistance at all. Adding impedance to the converter-fed
# sources while ignoring their current limits would be more detail and less
# truth.
#
# NOT modelled, declared instead: busbar impedance. Sub-milliohm against 20
# mohm of feeder is under 1 % of the answer for a whole model layer.
# =============================================================================
import math as mt
...
from config import (CABLE_TEMP_NIGHT_K, CONDUCTOR_RESISTIVITY_OHM_M,
                    ...
                    SSPC_MIN_TRIP_TIME_S, OCV_POLY_NMC, CELLS_SERIES,
                    BATTERY_R_INTERNAL_OHM, CONVERTER_FAULT_CURRENT_MULTIPLE,
...
def _pack_ocv_v(soc: float) -> float:
    """Pack open-circuit volts at a state of charge.

    The same Horner evaluation over the same ASCENDING coefficients as
    BatteryBank.open_circuit_voltage_v, kept here so protection does not have
    to import an asset. Six lines is cheaper than the coupling.
    """
    x = 2.0 * soc - 1.0
    result = 0.0
...
def source_fault_contribution_a(source_kind: str, feeder,
                                temperature_k: float,
                                soc: float = 1.0) -> float:
    """What ONE source can actually push into a bolted fault.

    Three kinds, each limited by something different. Only the battery is
    limited by impedance; the other two are limited by things that have
    nothing to do with resistance, which is why neither divides by it.

    @param source_kind  "battery", "pv" or "converter"
    @param soc          consulted for "battery" only
    @return             amps, always positive
    @raises ValueError  on an unknown kind — returning 0.0 for a typo would
        silently under-report a fault, and that is how protection gets
        under-sized.
    """
    if source_kind == "battery":
        # IMPEDANCE-limited. A fault sees OPEN-CIRCUIT voltage, because the
        # load is gone. The two resistances are in SERIES and both matter:
        # the pack's 14.3 mohm is 0.7x the cable's own.
        resistance = BATTERY_R_INTERNAL_OHM + feeder.resistance_ohm(temperature_k)
        return _pack_ocv_v(soc) / resistance

    rated_a = feeder_rated_current_a(feeder)

    if source_kind == "pv":
        # PHYSICS-limited. A photovoltaic cell is a current source once it is
        # off its knee: short it and you get Isc, whatever the circuit
        # resistance. No division here — the instinct from the battery case is
        # exactly wrong.
        return PV_SHORT_CIRCUIT_RATIO * rated_a

    if source_kind == "converter":
        # CONTROL-limited. A converter regulates current and refuses to exceed
        # its limit, so the cable is irrelevant to what it delivers.
        return CONVERTER_FAULT_CURRENT_MULTIPLE * rated_a

    raise ValueError(
        f"unknown source kind {source_kind!r}; expected "
        "'battery', 'pv' or 'converter'")


def is_pv_able_to_trip(feeder, temperature_k: float) -> bool:
    """Whether a PV array alone can reach its own device's trip threshold.

    @param temperature_k  accepted for symmetry with the other fault
        functions and genuinely unused: Isc does not depend on the cable.
    @note Expect FALSE, and expect that to be the finding. Isc is 1.15x rated
        against a 10x instantaneous threshold, so a PV array cannot trip the
        protection sized on its own feeder — it simply feeds the fault. This
        is a real property of PV systems and the reason they need a different
        protection philosophy from a battery.
    """
    del temperature_k                      # see @param
    rated_a = feeder_rated_current_a(feeder)
    threshold_a = SSPC_INSTANTANEOUS_TRIP_MULTIPLE * rated_a
    return source_fault_contribution_a("pv", feeder, 0.0) >= threshold_a
...
def total_fault_current_a(feeder, temperature_k: float,
    ...
    Sizing on one contribution is how a device ends up correctly rated for the
    wrong event.

    @param sources  which kinds are connected. Defaults to all three, which is

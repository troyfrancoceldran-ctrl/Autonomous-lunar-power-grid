...
class PVArray(PowerSource):
    """
    @brief Photovoltaic array; output tracks the environment's irradiance.
    ...
    def __init__(self, name="PV Array", area_m2=PV_AREA_M2,
        ...
        """
        @brief Construct a PV array from its nameplate specification.
        ...
        @note The config constants supply DEFAULTS only. Each instance stores
              its own values, so two arrays of different sizes can coexist.
        """
        self.name = name
    ...
    def available_power(self, t_hours: float, environment) -> float:
        """
        @brief Electrical power available from the array at time t.
        ...
        Computes P = G_sc * A * eta * f * phi(t), where G_sc is the solar
        constant at 1 AU (1361 W/m^2 — the Moon has no atmosphere, so the
        full extraterrestrial value reaches the panels) and phi(t) is the
        environment's irradiance fraction in [0, 1].

        Dimensionally: [W/m^2] * [m^2] * [-] * [-] * [-] = [W].

        @note The array knows nothing about lunar cycles. It asks the
              environment for a fraction and scales by it — which is why
              swapping in a real irradiance sensor requires no change here.
        """
...
class FissionSurfacePower(PowerSource):
    """
    @brief Kilopower-class fission reactor; constant output, indifferent to time.
    ...
    def __init__(self, name="FSP Reactor", rated_power_w=FSP_RATED_POWER_W,
        ...
        """
        @brief Construct a reactor from its nameplate specification.
    ...
    def available_power(self, t_hours: float, environment) -> float:
        """
        @brief Electrical power available from the reactor at time t.
        ...
        @note A reactor does not care what time it is or whether the sun is
              up. Both parameters are nevertheless required: the uniform
              signature is what lets power_bus.py iterate over mixed source
              types without a single isinstance() check. Removing them to
              silence the linter would break polymorphic dispatch.
        """
